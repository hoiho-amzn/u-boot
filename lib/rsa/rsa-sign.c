...
{
	if (engine)
		return rsa_engine_get_priv_key(keydir, name, engine, evpp);
	return rsa_pem_get_priv_key(keydir, name, evpp);
}
	     ...
	     uint8_t **sigp, uint *sig_len)
{
	...
	ret = rsa_get_priv_key(info->keydir, info->keyname, e, &pkey);
	if (ret)
		goto err_priv;
